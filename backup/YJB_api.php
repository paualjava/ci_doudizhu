...
class YJB_api extends MY_Controller {
	public $testParams;
	...
	public function  unittest()
	{
		...
		if (0) {
			$params["uid"]=$uid;
			$params["action"]="updateDiamondNumber";
			$params["number"]="3";
		...
		if (0) {
			unset($params);
			$params["action"]="borad_collection_delete";
			$params["uid"]=11;
			$params["token"]=$token;
			$params["uccid"]='2';
			$this->testParams = json_encode($params);
		...
		if (0) {
			unset($params);
			$params["action"]="getUserInfoStatistics";
			$params["uid"]=11;
			$params["t_uid"]=11;
			$params["token"]=$token;
			$this->testParams = json_encode($params);
		...
		if (0) {
			unset($params);
			$params["action"]="getUserInfoWithUserID";
			$params["uid"]=11;
			$params["t_uid"]=11;
			$params["token"]=$token;
			$this->testParams = json_encode($params);
		...
		if (0) {
			unset($params);
			$params["action"]="stat_club_board";
			$params["date"]="2016-02-22";
			$params["clubid"]="14";
			$this->testParams = json_encode($params);
			$this->index();
		}
		...
		if (0) {
			unset($params);
			$params["action"]="stat_money_date_count";
			$params["beginDate"]="2016-02-28 00:00:00";
			$params["endDate"]="2016-02-28 23:59:59";
			$params["uid"]=$uid;
			$params["token"]=$token;
			$this->testParams = json_encode($params);
			$this->index();
	...
	public function index()
	{
		...
		if (!empty($action)) {
			if(in_array($action,get_class_methods('YJB_api'))) {
				if ($action != "regist") {
	...
	function updateDiamondNumber($map) {
		$uid = isset($map->uid)?$map->uid:"";

		$token = isset($map->token)?$map->token:"";
		$userinfo = $this->checkLogin($uid, $token);
		$number = isset($map->number)?$map->number:0;
		$this->loadModel('user_model');
		$affectrows = $this->user_model->updateDiamondNumber($uid, $number,$userinfo);
		if ($affectrows) {
			$this->response->status = 0;
			$this->response->message = "修改钻石数量成功";
			$this->response->data = array($this->getUserInfo($uid));
		}else
		{
			$this->response->message = "修改钻石数量失败";
		}
		$this->echoSafeResponse();
	}
	...
	public function collectFriend($map) {
		$uid = isset($map->uid)?$map->uid:"";
		$token = isset($map->token)?$map->token:"";
		//$userinfo = $this->checkLogin($uid, $token);
		$t_uid = isset($map->t_uid)?$map->t_uid:"";
		$this->checkIsEmpty("目标用户id", $t_uid);
		$user = $this->user_model->collectFriend($uid,$t_uid);
		if ($user) {
			$isMyFriend = $this->checkIsMyFriend($uid, $t_uid);
			if ($isMyFriend) {
				//$user["clubids"] = $this->getClubIDSWithUserId($t_uid);
				$user["isMyFriend"] = TRUE;
			}else {
				$user["isMyFriend"] = FALSE;
			};
			$this->response->status = 0;
			$this->response->message = "收藏好友成功";
			$this->response->data = array($user);
			$this->echoSafeResponse();
		}else
		{
			$this->response->message = "收藏好友失败";
			$this->echoSafeResponse();
		}
	}
	public function getUserInfoStatistics($map) {
		$uid = isset($map->uid)?$map->uid:"";
		$token = isset($map->token)?$map->token:"";
		$userinfo = $this->checkLogin($uid, $token);
		$t_uid = isset($map->t_uid)?$map->t_uid:"";
		$this->checkIsEmpty("目标用户id", $t_uid);
		$user = $this->user_model->getUserInfoStatistics($t_uid);
		if ($user) {
			$isMyFriend = $this->checkIsMyFriend($uid, $t_uid);
			if ($isMyFriend) {
				//$user["clubids"] = $this->getClubIDSWithUserId($t_uid);
	 ...
	 */
	public function getPlayBack($map) {
		$bid = isset($map->bid)?$map->bid:"";
		$seq = isset($map->seq)?$map->seq:"";
		$this->checkIsEmpty("牌桌", $bid);
		$this->checkIsEmpty("牌桌的一局", $seq);
		$this->loadModel('board_model');
		$list = $this->board_model->getPlayBack($bid,$seq);
		if ($list) {
			$this->response->status = 0;
			$this->response->message = "获取到数据";
			$this->response->data = array($list);
			$this->echoSafeResponse();
		}else
		{
			$this->response->message = "没有符合条件的数据";
			$this->echoSafeResponse();
		}
	}
	public function getUserInfoWithUserID($map) {
		...
		$user = $this->getUserInfoDetail($t_uid);
		if ($user) {
			$isMyFriend = $this->checkIsMyFriend($uid, $t_uid);
			if ($isMyFriend) {
	...
	protected function getUserInfoDetail($uid,$closedToken = true,$clubsJoin=FALSE) {
		$this->loadModel("user_model");
		$arr = $this->user_model->userWithUserIDToDetial($uid);
		$userinfo = array();
		if (count($arr)) {
			$userinfo = $arr[0];
		}
	...
	public function club_create($map) {
		$uid = isset($map->uid)?$map->uid:"";
		$token = isset($map->token)?$map->token:"";
		$leixing = ($map->leixing>=1)?$map->leixing:1;
		$userinfo = $this->checkLogin($uid, $token);
		...
		$this->loadModel("club_model");
		$ok = $this->club_model->insert($name,$region_id,$uid_create,$leixing);
	...
	public function club_my($map) {
		$uid = isset($map->uid)?$map->uid:"";
		$token = isset($map->token)?$map->token:"";
		$userinfo = $this->checkLogin($uid, $token);

		$this->loadModel("club_model");
		$array = $this->club_model->myClubs($uid);
	...
	public function borad_collection_add($map) {
		$uid = isset($map->uid)?$map->uid:"";
		$token = isset($map->token)?$map->token:"";
		$userinfo = $this->checkLogin($uid, $token);
		...
		$this->loadModel("user_card_collection");
		$rowid = $this->user_card_collection->insert($uid,$bid,$hand_card,$common_card,$money,$type,$seq);
	...
	public function borad_collection_delete($map) {
		$uid = isset($map->uid)?$map->uid:"";
		$token = isset($map->token)?$map->token:"";
		$uccid = isset($map->token)?$map->uccid:"";
		$userinfo = $this->checkLogin($uid, $token);
		$this->checkIsEmpty("收藏ID", $uccid);
		$this->loadModel("user_card_collection");
		$ok = $this->user_card_collection->borad_collection_delete($uid,$uccid);
		if ($ok) {
			$this->response->status = 0;
			$this->response->message = "删除收藏成功";
			$this->echoSafeResponse();
		}else
		{
			$this->response->message = "删除收藏失败";
			$this->echoSafeResponse();
		}
	}
	public function borad_collection_list($map) {
		$uid = isset($map->uid)?$map->uid:"";
		$token = isset($map->token)?$map->token:"";
		$userinfo = $this->checkLogin($uid, $token);
		...
		$this->loadModel("user_card_collection");
		$res = $this->user_card_collection->show_user_collection($uid,$start,$count);
	...
	public function stat_board_history($map) {
		$uid = isset($map->uid)?$map->uid:"";
		$token = isset($map->token)?$map->token:"";
		$userinfo = $this->checkLogin($uid, $token);
		...
		$arr = $this->statBoardHistory_model->showHistoryWithDate($targetuid,$beginDate,$endDate,$startRow,$count,$clubid);
		if (count($arr)) {
			$this->response->status = 0;
			$this->response->message = "查询成功";
	...
	public function stat_money_date_count($map) {
		$uid = isset($map->uid)?$map->uid:"";
		$token = isset($map->token)?$map->token:"";
		$userinfo = $this->checkLogin($uid, $token);
		...
		$beginDate = isset($map->beginDate)?$map->beginDate:"";
		$endDate = isset($map->endDate)?$map->endDate:"";
		$beginDate=(strlen($beginDate)==8) ? preg_replace("/(\d{4})(\d{2})(\d{2})/is","\$1-\$2-\$3 00:00:01",$beginDate) : $beginDate;
		$endDate=(strlen($endDate)==8) ? preg_replace("/(\d{4})(\d{2})(\d{2})/is","\$1-\$2-\$3 23:59:59",$endDate) : $endDate;
		$this->checkIsEmpty("起始日期", $beginDate);
		$this->checkIsEmpty("结束日期", $endDate);

		$this->loadModel('statBoardHistory_model');
		$arr = $this->statBoardHistory_model->statMoneyDayCount($uid,$beginDate,$endDate);
		if (count($arr)) {
			$this->response->status = 0;
			$this->response->message = "查询成功";
			$this->response->data = $arr;
	...
	public function stat_club_board($map) {
		// 		$uid = isset($map->uid)?$map->uid:"";
		// 		$token = isset($map->token)?$map->token:"";
		// 		$userinfo = $this->checkLogin($uid, $token);
		...
		$this->loadModel('statBoardHistory_model');
		$arr = $this->statBoardHistory_model->clubBoardStat($date,$clubid,$start,$count);
		if (count($arr)) {
			$this->response->status = 0;
			$this->response->message = "查询成功";
			$this->response->data = $arr;
	...
	public function getBoardResult($map) {
		$bid = isset($map->bid)?$map->bid:"";
		...
		$this->loadModel('board_model');
		$allUsers = $this->board_model->getBoardResult($bid);
		if (count($allUsers)) {
			foreach ($allUsers as $key=>$val) {
				$user = $this->getUsersWithIDArray(array($val['uid']));
				if (count($user)) {
					$allUsers[$key]['avatar_thumb'] = $user[0]['avatar_thumb'];
				}
			}
			$res['info'] = $allUsers;
			$hands = $this->board_model->allhand($bid,$uid);
			$res['allhand'] = count($hands)?$hands[0]['num']:0;
			$max = $this->board_model->maxmoneypool($bid);
